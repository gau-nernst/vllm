...
def determine_expert_counts(
    ...
    if num_fused_shared_experts > 0 and not is_act_and_mul:
        raise ValueError(
            "fuse_shared_experts is only supported for gated MoE activations"
        )
...
def FusedMoE(
    ...
    Args:
        ...
        num_fused_shared_experts: Number of shared experts appended to the
                                  routed expert table. ``None`` disables
    ...
    if num_fused_shared_experts > 0 and shared_experts is not None:
        raise ValueError(
            "num_fused_shared_experts cannot be used with a separate "
